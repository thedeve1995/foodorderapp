...
<template>
  <div class="main-con">
    <h1 class="welcome"><img src="../assets/wok.png" width="150" alt=""></h1>
    <div class="form-con">
      <a class="lihat-antrian" href="#queue-con">Lihat Antrian</a>
      <form @submit.prevent="submitOrder">
        <h2 style="text-align: center;">Silahkan Order Disini</h2>
        <input type="text" placeholder="Nama Anda" v-model="nama">
        <input type="text" placeholder="Nomor Meja (Silahkan Lihat di Meja Anda)" v-model="table">
        <h2>Menu Makanan</h2>
        <div class="order" v-for="(input, index) in makananInputs" :key="index">
          <select style="width: 150px;" v-model="input.menu" @change="updateHarga(index)" id="menu">
            <option v-for="menuOption in menuOptions" :key="menuOption.id" :value="menuOption.name">
              {{ menuOption.name }} - Rp {{ menuOption.harga }}
            </option>
          </select>

          <input type="number" name="" id="" placeholder="jumlah" v-model="input.jumlah" class="jumlah">
          <button @click="addInputMakanan" v-if="index === makananInputs.length - 1" style="width: 40px;font-size: 25px;">
            + </button>
        </div>
        <h2>Menu Minuman</h2>
        <div class="order" v-for="(input, index) in minumanInputs" :key="index">
          <select style="width: 150px;" v-model="input.menu" @change="updateHargaMinuman(index)" class="menu">
            <option v-for="drinkOption in drinkOptions" :key="drinkOption.id" :value="drinkOption.name">
              {{ drinkOption.name }} - Rp {{ drinkOption.harga }}
            </option>
          </select>

          <input type="number" name="" id="" placeholder="jumlah" v-model="input.jumlah" class="jumlah">
          <button @click="addInputMinuman" v-if="index === minumanInputs.length - 1" style="width: 40px;font-size: 25px;">
            + </button>
        </div>
    ...
    <div id="queue-con">
      ...
      <div class="card-con">
        <div class="card" v-for="(data, index) in datas" :key="data.id">
          <span>Nama Pemesan : {{ data.nama }}</span> <br>
          <span>Nomor Meja : {{ data.table }}</span> <br>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(x, index) in data.order" :key="x.id">
                ...
                <td>{{ x.jumlah }}</td>
                <td>Rp {{ x.harga }}</td>
                <td><button :style="{ backgroundColor: x.diantar ? 'rgb(2, 121, 240)' : 'rgb(253, 127, 0)' }" class="actBtn" > {{ !x.diantar? "Belum" : "sudah" }} </button></td>
              </tr>
            </tbody>
          </table>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(x, index) in data.minuman" :key="x.id">
                ...
                <td>{{ x.jumlah }}</td>
                <td>Rp {{ x.harga }}</td>
                <td><button class="actBtn" :style="{ backgroundColor: x.diantar ? 'rgb(2, 121, 240)' : 'rgb(253, 127, 0)' }"> {{ !x.diantar? "Belum" : "sudah" }} </button></td>
              </tr>
            </tbody>
          </table>

          <!-- <div class="btn-group">
            <button @click="completeOrder(data.id)">Selesai</button>
          </div> -->
        </div>
      </div>
    </div>
...
.lihat-antrian {
  ...
  border: none;
  outline: none;
  background-color: rgba(247, 198, 1);
  margin-bottom: 20px;
  width: 28%;
}
...
.main-con #queue-con .card-con .card {
  background-color: rgba(247, 198, 1, 0.7);
  padding: 20px;
  border-radius: 10px;
}
