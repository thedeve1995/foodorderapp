...
const checkAllDelivered = (array) => {
  return array.every(item => item.diantar);
};

const showAlert = () => {
  alert("Mohon selesaikan pengantaran semua pesanan sebelum menyelesaikan pesanan!");
};
</script>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(x, index) in data.order" :key="x.id">
                ...
                <td>{{ x.jumlah }}</td>
                <td>Rp {{ x.harga }}</td>
                <td><button :style="{ backgroundColor: x.diantar ? 'rgb(2, 121, 240)' : 'rgb(253, 127, 0)' }"
                    class="actBtn" @click="diantar(data.id, index)"> {{ !x.diantar ? "Belum" : "sudah" }} </button></td>
              </tr>
            </tbody>
          </table>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(x, index) in data.minuman" :key="x.id">
                ...
                <td>{{ x.jumlah }}</td>
                <td>Rp {{ x.harga }}</td>
                <td><button class="actBtn"
                    :style="{ backgroundColor: x.diantar ? 'rgb(2, 121, 240)' : 'rgb(253, 127, 0)' }"
                    @click="minumanDiantar(data.id, index)"> {{ !x.diantar ? "Belum" : "sudah" }} </button></td>
              </tr>
            </tbody>
          </table>

          <div class="btn-group">
            <button style="background-color: rgb(0, 119, 255);" @click="checkAllDelivered(data.order) && checkAllDelivered(data.minuman) ? completeOrder(data.id) : showAlert()">Selesai</button>
            <button style="background-color: rgba(255, 0, 0, 0.574);" @click="deleteOrder(data.id)">Delete </button>
          </div>
        </div>
      </div>
    </div>
